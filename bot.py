...
async def help_command(message: types.Message):
    reply = Response('🧙🏼‍♂', "<b>@WednesdayWizardBot</b>! 💬")
    lines = help_msg()
    lines.append(reply.separator)
    reply.lines = lines
    reply.add_lines()
...
async def show_command(message: types.Message):
    if message.is_command():
        ...
        lines = [f"LINK_ID: <b>{i}</b> | {link.time} | {icon_number(link.votes)} 👍\n{link.url}"
                 for i, link in enumerate(db.get()['links'])]
        lines.reverse()
        lines = lines[:length]
        reply.lines = lines
        reply.add_lines()
...
@dp.message_handler(commands=['done', 'like', 'tick'])
